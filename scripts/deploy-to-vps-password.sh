...
if ! sshpass -p "$VPS_PASSWORD" ssh -p "$VPS_PORT" -o ConnectTimeout=10 -o StrictHostKeyChecking=no "$VPS_USER@$VPS_HOST" "echo 'SSH接続成功'" 2>/dev/null; then
...
sshpass -p "$VPS_PASSWORD" scp -P "$VPS_PORT" -o StrictHostKeyChecking=no "/tmp/${PROJECT_NAME}.tar.gz" "$VPS_USER@$VPS_HOST:/opt/url-template/"
...
echo "  sshpass -p '$VPS_PASSWORD' ssh $VPS_USER@$VPS_HOST 'cd /opt/url-template && ./scripts/setup-letsencrypt.sh $DOMAIN admin@$DOMAIN'" 